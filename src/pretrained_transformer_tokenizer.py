...
class PretrainedTransformerTokenizer(Tokenizer):
    ...
    def tokenize(self, text: str, **kwargs) -> List[Token]:
        ...
        if self._model_name.startswith('studio-ousia/mluke'):
            params['entity_spans'] = kwargs.get("entity_spans", [])

        encoded_tokens = self.tokenizer.encode_plus(**params)
        ...
        if self._model_name.startswith('studio-ousia/mluke'):
            entity_ids, entity_position_ids, entity_token_type_ids = (
                encoded_tokens["entity_ids"],
                encoded_tokens["entity_position_ids"],
                encoded_tokens["entity_token_type_ids"],
            )
        ...
        tokens = []
        if self._model_name.startswith('studio-ousia/mluke'):
            for token_id, token_type_id, entity_id, entity_position_id, entity_token_type_id, special_token_mask, offsets in zip(
                    token_ids, token_type_ids, entity_ids, entity_position_ids, entity_token_type_ids,
                    special_tokens_mask, token_offsets):
                ...
                tokens.append(
                    Token(
                        ...
                        type_id=token_type_id,
                        entity_id=entity_id,
                        entity_position_id=entity_position_id,
                        entity_token_type_id=entity_token_type_id,
                        idx=start,
                        idx_end=end,
                    )
                )
    ...
    def _intra_word_tokenize(
            self, string_tokens: List[str], entities=None
