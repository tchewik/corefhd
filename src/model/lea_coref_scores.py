...
@Metric.register("lea_coref_scores")
class LeaCorefScores(Metric):

    supports_distributed = True

    def __init__(self, allow_singletons=False) -> None:
        self._available_scorers = (Scorer.lea,)  # (Scorer.muc, Scorer.b_cubed, Scorer.ceafe)
        self.scorers = [Scorer(m) for m in self._available_scorers]
    ...
    def reset(self):
        self.scorers = [Scorer(metric) for metric in self._available_scorers]
...
class Scorer:
    ...
    def update(self, predicted, gold, mention_to_predicted, mention_to_gold):
        if self.metric in [self.ceafe, self.lea]:
    ...
    @staticmethod
    def lea(clusters, gold_clusters):
        """
        Computes the Link-based Entity Aware Metric (LEA) for evaluating coreference.
        The mention identification effect does not apply to LEA recall and precision.
        <https://aclanthology.org/P16-1060.pdf>
        """
        ...
        precision, p_weight = _lea(gold_clusters, clusters)
        recall, r_weight = _lea(clusters, gold_clusters)
        return precision, p_weight, recall, r_weight
