...
class PretrainedTransformerMismatchedIndexer(TokenIndexer):
    ...
    def tokens_to_indices(self, tokens: List[Token], vocabulary: Vocabulary) -> IndexedTokenList:
        ...
        max_seq_length = 512
        offsets = [x if (x is not None) and (x[0] < max_seq_length and x[1] < max_seq_length)
                   else (-1, -1) for x in offsets]

        output: IndexedTokenList = {
            "token_ids": [t.text_id for t in wordpieces][:max_seq_length],
            "mask": [True] * min(len(tokens), max_seq_length),  # for original tokens (i.e. word-level)
            "type_ids": [t.type_id for t in wordpieces][:max_seq_length],
            "offsets": offsets[:max_seq_length],
            "wordpiece_mask": [True] * min(len(wordpieces), max_seq_length),  # for wordpieces (i.e. subword-level)
        }

        if self._model_name.startswith('studio-ousia/mluke'):
            # We need to match the named entities with the wordpieces
            # The wordpieces are already tokenized by the tokenizer
            entity_ids = []
            ...
            for token, offset in zip(tokens, offsets):
                if not offset == (0, 0):
                    if token.ent_type_[0] == 'B':
                        entity_ids.append(0)
                        entity_attention_mask.append(1)
                        current_positions = list(range(offset[0], offset[1] + 1))
                        entity_position_ids.append(current_positions + [-1] * (max_entity_len - len(current_positions)))

                    elif token.ent_type_[0] == 'I':
                        current_positions += list(range(offset[0], offset[1] + 1))
                        entity_position_ids[-1] = current_positions + [-1] * (max_entity_len - len(current_positions))

            # Dummy padding
            max_entities = 100
            for j in range(len(entity_ids), max_entities):
                entity_ids.append(-1)
                entity_attention_mask.append(0)
                entity_position_ids.append([-1] * max_entity_len)

            output['entity_ids'] = entity_ids[:min(len(entity_ids), max_entities)]
            output['entity_attention_mask'] = entity_attention_mask[:min(len(entity_attention_mask), max_entities)]
            output['entity_position_ids'] = entity_position_ids[:min(len(entity_position_ids), max_entities)]
