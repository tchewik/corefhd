...
from .util import make_coref_instance

logger = logging.getLogger(__name__)


@DatasetReader.register("light_coref")
class LightConllCorefReader(DatasetReader):
    """
    ...
    Comparing to the original connl-u format reader, it is wired to also read the additional
    paragraph-splitting markers.

    Returns a `Dataset` where the `Instances` have FIVE fields : `text`, a `TextField`
    containing the full document text, `spans`, a `ListField[SpanField]` of inclusive start and
    end indices for span candidates, a `ListField` of paragraph starters
    and `metadata`, a `MetadataField` that stores the instance's original text.
    For data with gold cluster labels, we also include the original `clusters`
    ...
    def __init__(
            ...
            span_extraction_strategy: str = 'all',
            read_rst_annotation: bool = False,
            rst_annotation_dir: str = 'data/rst_annotation',
            **kwargs,
    ) -> None:
        ...
        self._span_extraction_strategy = span_extraction_strategy
        self._read_rst_annotation = read_rst_annotation
        self._rst_annotation_dir = rst_annotation_dir if self._read_rst_annotation else None
    ...
    def _read(self, file_path: str):
        ...
        for sentences in ontonotes_reader.dataset_document_iterator(file_path):
            ...
            yield self.text_to_instance(sentences=sentences,
    ...
    def text_to_instance(
        ...
        return make_coref_instance(
            ...
            self._span_extraction_strategy,
            nlp_annot,
            self._rst_annotation_dir
        )
