...
class OntonotesSentence:
    ...
    parse_tree : tuple
        (! In original Ontonotes, it is the nltk.Tree.)
        Tuple (head, rel) with information about a token from dependency tree.
    ...
    coref_spans : `Set[TypedSpan]`
        ...
        are `inclusive`.
    new_paragraph : `List[Optional[bool]]`
        Marker of the beginning of a new paragraph
    """
    ...
    def __init__(
            ...
            coref_spans: Set[TypedSpan],
            new_paragraph: List[Optional[bool]],
            named_entities_idxs: List[Optional[int]] = None,
    ) -> None:
        self.document_id = document_id
        self.sentence_id = sentence_id
        self.words = words
        self.pos_tags = pos_tags
        self.parse_tree = [eval(parse) for parse in parse_tree]
        ...
        self.new_paragraph_mark = False
        self.named_entities_idxs = named_entities_idxs
...
class Ontonotes:
    ...
    def dataset_document_iterator(self, file_path: str) -> Iterator[List[OntonotesSentence]]:
        ...
        with codecs.open(file_path, "r", encoding="utf8") as open_file:
            ...
            for line in open_file:
                line = line.strip()
                if line.startswith('#new paragraph'):
                    self.new_paragraph_mark = True
    ...
    def _conll_rows_to_sentence(self, conll_rows: List[str]) -> OntonotesSentence:
        ...
        new_paragraph: List[int] = []
        named_entities: List[str] = []
        named_entities_idxs: List[Tuple[int, int]] = []
        self.new_paragraph_mark = False

        for index, row in enumerate(conll_rows):
            conll_components = row.split('\t')

            document_id = conll_components[0].strip()
            sentence_id = int(conll_components[1])
            word = conll_components[3].strip()
            pos_tag = conll_components[4].strip()
            parse_piece = conll_components[5].strip()
            ...
            named_entities.append(conll_components[10].strip())

            if conll_components[10] != '*':
                # B_* means the start of a named entity
                # I_* means the continuation of a named entity
                # * means not a named entity
                if conll_components[10].startswith('B'):
                    named_entities_idxs.append((index, index))
                elif conll_components[10].startswith('I'):
                    named_entities_idxs[-1] = (named_entities_idxs[-1][0], index)

            new_paragraph.append(self.new_paragraph_mark)
            self.new_paragraph_mark = False
