...
@Predictor.register("light_coreference_resolution")
class LightCorefPredictor(Predictor):
    """
    ...
    def predict_isanlp(self, text, tokens, sentences, lemma, postag, syntax_dep_tree, entities, rst,
                       _paragraph_markers: bool = True):
        doc_id = 'current_doc'
        ...
        for i, sentence in enumerate(sentences):
            result = []
            if _paragraph_markers:
                ontonotes_reader.new_paragraph_mark = False
                if sentence.begin == 0:
                    ontonotes_reader.new_paragraph_mark = True
                else:
                    marker_location = tokens[sentence.begin].begin - 1
                    if text[marker_location: marker_location + 1] == '\n':
                        ontonotes_reader.new_paragraph_mark = True

            for j, token in enumerate(tokens[sentence.begin:sentence.end]):
                text = token.text
                token_pos = postag[i][j]
                token_parse = str((syntax_dep_tree[i][j].parent, syntax_dep_tree[i][j].link_name))
                token_lemma = lemma[i][j]
                token_framenet_ids = '-'
                token_sense = '-'
                token_speaker = 'Speaker#1'
                token_entity = '*'
                token_srl = '*'
                token_coref = '-'

                # collect NER results in BIO format
                for entity in entities:
                    if entity.begin == sentence.begin + j:
                        token_entity = 'B_' + entity.tag
                    elif entity.begin < sentence.begin + j < entity.end:
                        token_entity = 'I_' + entity.tag

                result.append(
                    '\t'.join(map(str, [doc_id, 0, j, text, token_pos, token_parse, token_lemma, token_framenet_ids,
                                        token_sense,
                                        token_speaker, token_entity, token_srl, token_coref])))

            ontonotes_sentences.append(ontonotes_reader._conll_rows_to_sentence(result))

        if rst is not None:
            nlp_annot = {'tokens': tokens, 'rst': rst}
        else:
            nlp_annot = None

        instance = self._dataset_reader.text_to_instance(ontonotes_sentences, nlp_annot=nlp_annot)
        return self.predict_instance(instance)
