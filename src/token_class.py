...
class Token:
    ...
    entity_position_id = Optional[int]
    ...
    def __init__(
        ...
        self.entity_position_id = entity_position_id
...
def show_token(token: Token) -> str:
    return (
        ...
        f"(entity_id: {token.entity_id}) "
        f"(entity_position_id: {token.entity_position_id}) "
        f"(entity_type_id: {token.entity_type_id})"
    )