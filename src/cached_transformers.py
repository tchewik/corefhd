    ...
    **kwargs,
    ...
    if transformer is None:
        if not load_weights:
            ...
            if model_name == 'yandex/RuLeanALBERT':
                from src.ruleanalbert.src.models.albert import LeanAlbertConfig, LeanAlbertForPreTraining
                from transformers.models.albert import AlbertTokenizerFast
                import torch
                import os

                albert_path = kwargs.get('leanalbert_src_path', os.path.join('models', 'ruleanalbert'))
                tokenizer = AlbertTokenizerFast.from_pretrained(os.path.join(albert_path, 'tokenizer'))
                config = LeanAlbertConfig.from_pretrained(os.path.join(albert_path, 'config.json'))
                transformer = LeanAlbertForPreTraining(config)
                transformer.resize_token_embeddings(len(tokenizer))
                checkpoint_path = os.path.join(albert_path, 'state.pth')
                checkpoint = torch.load(checkpoint_path, map_location='cpu')['model']
                transformer.load_state_dict(checkpoint)
                transformer = transformer.albert
    ...
    if make_copy and model_name != 'yandex/RuLeanALBERT':
        import copy
